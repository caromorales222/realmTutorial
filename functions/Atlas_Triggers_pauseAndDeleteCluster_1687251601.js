...
exports = async function() {
  ...
  clusters.forEach(async function (cluster){
    ...
    if(cluster.name.toLowerCase().endsWith("topause") || cluster.name.toLowerCase().startsWith("eci3")){
      message = "paused";
    ...
    else if (cluster.name.toLowerCase().endsWith("todelete")){
      message = "deleted"

      response = await context.http.delete(argsClusterAction);
    }
